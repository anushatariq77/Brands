import React from 'react';

// const data =[
//   {
//     name:'
//   }
// ]
const Grid=()=>{
  return(
    <div className = "row" style ={{ display : 'flex'}}>
    <div className="column" style={{  flex: '33.33%',
      padding: '5px'}}>
          <div className="ui card" style={{margin:'50px 0 0 10px' , boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)' }}>
            <div className="ui slide masked reveal image"  >
              <img src="ak.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="ak.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header"  style={{color:"#D79922"}}>ALKARAM STUDIO </a>
              <hr ></hr>
            </div>
          </div>
    
          <div className="ui card" style={{margin:'50px 0 0 10px' , boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)' }}>
           
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header"  style={{color:"#F13C20"}}>ASIM JOFA</a>
              <hr ></hr>
            </div>
            <div className="ui slide masked reveal image"  >
              <img src="as.png" className="visible content" style = {{ height : '200px'}} />
              <img src="as.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
          </div>

          <div className="ui card" style={{margin:'50px 0 0 10px' , boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)' }}>
            <div className="ui slide masked reveal image"  >
              <img src="ch.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="ch.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header"  style={{color:"#D79922"}}>CHARIZMA </a>
              <hr ></hr>
            </div>
          </div>

      <div className="ui card" style={{margin:'50px 0 0 10px' , boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)' }}>
        <div className="content" style={{width:"40vh", textAlign:"center"}}>
             <a className="header"  style={{color:"#F13C20"}}>DHANAK</a>
             <hr ></hr>
           </div>
           <div className="ui slide masked reveal image"  >
             <img src="dn.png" className="visible content" style = {{ height : '200px'}} />
             <img src="dn.png" className="hidden content" style = {{ height : '200px'}}/>
           </div>
         </div>

         <div className="ui card" style={{margin:'50px 0 0 10px' , boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)' }}>
            <div className="ui slide masked reveal image"  >
              <img src="OT.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="OT.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header"  style={{color:"#D79922"}}>OUTFITTERS</a>
              <hr ></hr>
            </div>
          </div>

      <div className="ui card" style={{margin:'50px 0 0 10px' , boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)' }}>
        <div className="content" style={{width:"40vh", textAlign:"center"}}>
             <a className="header"  style={{color:"#F13C20"}}>J. JUNAID JAMSHED</a>
             <hr ></hr>
           </div>
           <div className="ui slide masked reveal image"  >
             <img src="J.png" className="visible content" style = {{ height : '200px'}} />
             <img src="J.png" className="hidden content" style = {{ height : '200px'}}/>
           </div>
         </div>

    </div>
    
    
    
    <div className="column" style={{  flex: '33.33%',
        padding: '5px'}}>
          
    
          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="ui slide masked reveal image"  style = {{ height: '200px'}}>
              <img src="kd.png" className="visible content" />
              <img src="kd.png" className="hidden content" />
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header"  style={{color:"#D79922"}}>KHAADI</a>
              <hr ></hr>          
            </div>
          </div>
    
          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
           
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header"  style={{color:"#F13C20"}}>BONANZA</a>
              <hr ></hr>
            </div>
            <div className="ui slide masked reveal image"  >
              <img src="bz.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="bz.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
          </div>

          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="ui slide masked reveal image"  style = {{ height: '200px'}}>
              <img src="ed.png" className="visible content" />
              <img src="ed.png" className="hidden content" />
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header"  style={{color:"#D79922"}}>EDENROBE</a>
              <hr ></hr> 
            </div>
          </div>

          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>   
           <div className="content" style={{width:"40vh", textAlign:"center"}}>
             <a className="header"  style={{color:"#F13C20"}}>LIMELIGHT</a>
             <hr ></hr>
           </div>
           <div className="ui slide masked reveal image"  >
             <img src="lm.png" className="visible content" style = {{ height : '200px'}}/>
             <img src="lm.png" className="hidden content" style = {{ height : '200px'}}/>
           </div>
         </div>

         <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="ui slide masked reveal image"  style = {{ height: '200px'}}>
              <img src="BS.png" className="visible content" />
              <img src="BS.png" className="hidden content" />
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header"  style={{color:"#D79922"}}>BIN SAEED</a>
              <hr ></hr>          
            </div>
          </div>

          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>   
        <div className="content" style={{width:"40vh", textAlign:"center"}}>
             <a className="header"  style={{color:"#F13C20"}}>CHEN ONE</a>
             <hr ></hr>
           </div>
           <div className="ui slide masked reveal image"  >
             <img src="CO.png" className="visible content" style = {{ height : '200px'}}/>
             <img src="CO.png" className="hidden content" style = {{ height : '200px'}}/>
           </div>
         </div>

    

    </div>
    
    
    
    
     <div className = "column" style={{  flex: '33.33%',
      padding: '5px'}}>
          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="ui slide masked reveal image"  >
              <img src="sp.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="sp.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header" style={{color:'#AC3B61'}}>SAPPHIRE</a>
              <hr ></hr>         
            </div>
         </div>

          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>   
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
            <a className="header" style={{color:'#5D0015'}}>NISHAT LINEN</a>
            <hr ></hr>
            </div>
            <div className="ui slide masked reveal image"  >
              <img src="nl.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="nl.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
          </div>

          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="ui slide masked reveal image"  >
              <img src="mb.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="mb.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header" style={{color:'#AC3B61'}}>MARIA B</a>
              <hr ></hr>         
            </div>
         </div> 

         <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>   
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
            <a className="header" style={{color:'#5D0015'}}>MOTIFZ</a>
            <hr ></hr>
            </div>
            <div className="ui slide masked reveal image"  >
              <img src="mf.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="mf.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
          </div>

        </div>

        <div className = "column" style={{  flex: '33.33%',
      padding: '5px'}}>
          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="ui slide masked reveal image"  >
              <img src="gul.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="gul.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header" style={{color:'#AC3B61'}}>GUL AHMED</a>
              <hr ></hr>             
            </div>
         </div>

          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
          <a className="header" style={{color:'#5D0015'}}>SANA SAFINAZ</a>
              <hr ></hr>
            </div>
            <div className="ui slide masked reveal image"  >
              <img src="SN.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="SN.png" className="hidden content" style = {{ height : '200px'}} />
            </div>
          </div>

          <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="ui slide masked reveal image"  >
              <img src="et.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="et.png" className="hidden content" style = {{ height : '200px'}}/>
            </div>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
              <a className="header" style={{color:'#AC3B61'}}>ETHNIC</a>
              <hr ></hr>             
            </div>
         </div>

         <div className="ui card" style={{margin:'50px 0 0 10px', boxShadow:'-1px 4px 26px 5px rgba(0,0,0,0.75)'}}>
            <div className="content" style={{width:"40vh", textAlign:"center"}}>
          <a className="header" style={{color:'#5D0015'}}>EGO</a>
              <hr ></hr>
            </div>
            <div className="ui slide masked reveal image"  >
              <img src="EG.png" className="visible content" style = {{ height : '200px'}}/>
              <img src="EG.png" className="hidden content" style = {{ height : '200px'}} />
            </div>
          </div>

        </div>

        </div>


      

  );
};
export default Grid;